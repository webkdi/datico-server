...
async function enterClientFromWebhook(webhookBody) {
  ...
  const archivedVariables = await db.archiveVariables(clientId,variablesJson);
...
async function getClientsFromSb(clientId) {
  ...
  for (const clientId of idArray) {
    try {
      let insertedRows = await db.insertIgnoreClient(clientId);
